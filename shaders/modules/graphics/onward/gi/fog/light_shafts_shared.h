...
MR_RWTexture3D< float > LightShaftsRW : MR_RW_Texture0;

constants : MR_ConstantBuffer5
{
	float3 constJitter;

	uint3  constLightShaftsSize;
	float3 constLightShaftsInvSize;
	float4 constLightShaftsParams;
}

@ifndef GFX_NO_SHADOWS

MR_Sampler2DArrayCmp DownsampledShadowMap : MR_Texture0 { filter = linear point, wrap = clamp, compare = > };
MR_Sampler2D DownsampledDepth : MR_Texture1 { filter = point, wrap = clamp };

float GetShadow( float4 aShadowPos )
{
	return MR_SampleLod0Cmp(DownsampledShadowMap, aShadowPos.xyz, aShadowPos.w).x;
}
...
{
	float3 worldPosMax = MR_ClipToWorld((outputCoordinate + 0.5) * float2(2,-2) * constLightShaftsInvSize.xy + float2(-1,1), maxDepth );
	...
	float rayStartT = 0;
	float rayEndT = constLightShaftsParams.z;
	...
	float stepsPerDepthSlice = rayNumSamples * constLightShaftsInvSize.z;
	...
	for( ; i <= t_0; i+=1 )
	{
		if( i > finalSliceStep )
		{
			...
			++depthSlice;
			if( depthSlice >= constLightShaftsSize.z )
			{
				return;
			}
