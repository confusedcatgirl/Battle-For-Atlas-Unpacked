...
float3 OW_OpacityLighting_GetDiffReflectance(in float3 aBaseColor, in OGfx_Material aMaterial)
{
	return aBaseColor * (1.0 - aMaterial.metalness);
}

float3 OW_OpacityLighting_GetDiff(in OGfx_Material aMaterial, in float3 aReflectance, in float3 aL, in float3 aN)
{
...
float3 OW_OpacityLighting_GetSpecReflectance(in float3 aBaseColor, in OGfx_Material aMaterial)
{
	const float ourDielectricNormalReflectance = 0.04;
	return lerp(ourDielectricNormalReflectance, aBaseColor, aMaterial.metalness);
}

float3 OW_OpacityLighting_GetSpec(in OGfx_Material aMaterial, in float3 aReflectance, in float3 aL, in float3 aN, in float3 aV)
{
	float dotNL = dot(aN, aL);
	aL = (dotNL > 0.0 ? aL : -aL);
	dotNL = saturate(dotNL > 0.0 ? dotNL * aMaterial.opacity : dotNL * aMaterial.opacity - dotNL);
...
// Specialized
float3 OW_OpacityLighting_GetSpec(in OGfx_Material aMaterial, in float3 aReflectance, in float3 aL, in float3 aN, in float3 aV, in float aGV, in float aAlphaSquared)
{
	float dotNL = dot(aN, aL);
	aL = (dotNL > 0.0 ? aL : -aL);
	dotNL = saturate(dotNL > 0.0 ? dotNL * aMaterial.opacity : dotNL * aMaterial.opacity - dotNL);
