...
float3 OW_Noise_Mod289(float3 x)
{
  return x - floor(x * (1.0f / 289.0f)) * 289.0f;
}

float4 OW_Noise_Mod289(float4 x)
{
  return x - floor(x * (1.0f / 289.0f)) * 289.0f;
}

float4 OW_Noise_Permute(float4 x) 
{
     return OW_Noise_Mod289(((x * 34.0f) + 1.0f) * x);
}

float4 OW_Noise_TaylorInvSqrt(float4 r)
{
    return 1.79284291400159f - 0.85373472095314f * r;
}

float OW_Noise_SNoise(float3 v)
{
    ...
    // Permutations
    i = OW_Noise_Mod289(i); 
    float4 p = OW_Noise_Permute( OW_Noise_Permute( OW_Noise_Permute( 
    ...
    //Normalise gradients
    float4 norm = OW_Noise_TaylorInvSqrt(float4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));
    p0 *= norm.x;
    p1 *= norm.y;
    p2 *= norm.z;
    p3 *= norm.w;
